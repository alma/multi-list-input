...
var $author$project$ListType$Emails = 1;
var $author$project$ListType$IPS = 2;
var $author$project$ListType$Other = 3;
var $author$project$ListType$Tags = 0;
var $author$project$ListType$fromString = function (list_type) {
	switch (list_type) {
		case 'tags':
			return 0;
		case 'emails':
...
var $larribas$elm_multi_input$MultiInput$init = function (id) {
	return {J: id, p: false, aH: ''};
};
var $author$project$ListType$tagId = function (list_type) {
	switch (list_type) {
		case 0:
			return 'tags-input';
		case 1:
			return 'emails-input';
		case 2:
			return 'ip-addresses-input';
		default:
			return 'items-input';
	}
};
var $author$project$Main$newState = function (list_type) {
	return $larribas$elm_multi_input$MultiInput$init(
		$author$project$ListType$tagId(list_type));
};
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$init = function (flags) {
	var list_type = $author$project$ListType$fromString(flags.n);
	return _Utils_Tuple2(
		{
			w: flags.K,
			n: list_type,
			D: flags.K,
			z: $author$project$Main$newState(list_type),
			I: true
		},
...
var $author$project$Main$MultiInputMsg = function (a) {
	return {$: 0, a: a};
};
...
var $author$project$Main$itemsSaved = _Platform_incomingPort('itemsSaved', $elm$json$Json$Decode$value);
...
var $larribas$elm_multi_input$MultiInput$subscriptions = function (state) {
	return state.p ? $elm$browser$Browser$Events$onAnimationFrame(
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$Main$MultiInputMsg,
				$larribas$elm_multi_input$MultiInput$subscriptions(model.z)),
				$author$project$Main$itemsSaved($author$project$Main$Saved)
			]));
};
var $author$project$Main$defaultSeparators = _List_fromArray(
	['\n', '\t', ' ', ',']);
...
var $author$project$Main$itemsUpdate = _Platform_outgoingPort(
	'itemsUpdate',
...
var $elm$regex$Regex$Match = F4(
	function (match, index, number, submatches) {
		return {aB: index, aE: match, aI: number, aP: submatches};
	});
			...
			case 0:
				return _Utils_Tuple3(
					...
					state.p ? A2(
						...
						$elm$browser$Browser$Dom$focus(state.J)) : $elm$core$Platform$Cmd$none);
			case 2:
				var key = msg.a;
			...
			case 4:
				...
				var separatorRegex = A2(
					...
					$elm$regex$Regex$fromString(
						A2($elm$core$String$join, '|', conf.aO)));
...
var $author$project$ListType$ipv4_regex = '^((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])$';
var $author$project$ListType$ipv6_regex = '^([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{1,4}){2,2}%[0-9a-zA-Z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])$';
...
var $author$project$ListType$matches = function (regex) {
	var validRegex = A2(
		$elm$core$Maybe$withDefault,
		$elm$regex$Regex$never,
...
var $author$project$ListType$validator = function (list_type) {
	switch (list_type) {
		case 0:
			return $author$project$ListType$matches('^[a-z0-9:_]+$');
		case 1:
			return $author$project$ListType$matches('^\\w+([\\.\\+]?[\\w\\-])*@[a-z0-9]+(\\.?[-a-z0-9]+)*\\.[a-z]+$');
		case 2:
			return function (ip) {
				return A2($author$project$ListType$matches, $author$project$ListType$ipv6_regex, ip) || A2($author$project$ListType$matches, $author$project$ListType$ipv4_regex, ip);
			};
		default:
			return $author$project$ListType$matches('^[a-zA-Z0-9]+$');
	}
};
var $author$project$Main$update = F2(
	function (msg, model) {
		if (!msg.$) {
			var multiInputMsg = msg.a;
			if (model.I) {
				var _v1 = A4(
					$larribas$elm_multi_input$MultiInput$update,
					{aO: $author$project$Main$defaultSeparators},
					multiInputMsg,
					model.z,
					model.w);
				var nextState = _v1.a;
				var nextItems = _v1.b;
				var nextCmd = _v1.c;
				var acceptedItems = A2(
					$elm$core$List$filter,
					$author$project$ListType$validator(model.n),
					nextItems);
				var shouldSave = (nextState.aH === '') && ((!_Utils_eq(nextItems, model.w)) && (_Utils_eq(nextItems, acceptedItems) && (!_Utils_eq(acceptedItems, model.D))));
				var savedItems = shouldSave ? acceptedItems : model.D;
				var updatedList = shouldSave ? $author$project$Main$itemsUpdate(acceptedItems) : $elm$core$Platform$Cmd$none;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							...
							D: savedItems,
							z: nextState,
							I: _Utils_eq(updatedList, $elm$core$Platform$Cmd$none)
						}),
					$elm$core$Platform$Cmd$batch(
						_List_fromArray(
							[
								A2($elm$core$Platform$Cmd$map, $author$project$Main$MultiInputMsg, nextCmd),
								updatedList
							])));
			} else {
...
var $author$project$ListType$placeholder = function (listType) {
	switch (listType) {
		case 0:
			return 'Ajouter des tags';
		case 1:
			return 'Ajouter des adresses email';
		case 2:
			return 'Ajouter des adresses IP';
		default:
			return 'Ajouter votre liste';
	}
};
			...
			_List_fromArray(
				[
					...
					_Utils_ap(
						_List_fromArray(
							[
								$elm$html$Html$Attributes$value(state.aH),
								$elm$html$Html$Attributes$placeholder(conf.aM),
								$elm$html$Html$Attributes$rows(1),
								$elm$html$Html$Attributes$id(state.J),
								$elm$html$Html$Events$onInput(
								A2($elm$core$Basics$composeL, conf.aS, $larribas$elm_multi_input$MultiInput$InputChanged)),
								$elm$html$Html$Events$onBlur(
								conf.aS(
									$larribas$elm_multi_input$MultiInput$TextareaBlurred(state.aH))),
								$larribas$elm_multi_input$MultiInput$onKeyDown(
								A2($elm$core$Basics$composeL, conf.aS, $larribas$elm_multi_input$MultiInput$KeyDown))
							]),
						customAttributes),
					_List_Nil)
				]));
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Events$onClick(
							conf.aS($larribas$elm_multi_input$MultiInput$FocusElement))
						]),
					_Utils_ap(
						A2(
							$elm$core$List$map,
...
var $author$project$Main$view = function (model) {
	return A4(
		...
		{
			aD: $author$project$ListType$validator(model.n),
			aM: $author$project$ListType$placeholder(model.n),
			aS: $author$project$Main$MultiInputMsg
		},
		_List_Nil,
		model.w,
		model.z);
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{aC: $author$project$Main$init, aQ: $author$project$Main$subscriptions, aT: $author$project$Main$update, aU: $author$project$Main$view});
_Platform_export({'Main':{'init':$author$project$Main$main(
	A2(
		...
		A2($elm$json$Json$Decode$field, 'list_type', $elm$json$Json$Decode$string)))(0)}});}(this));